...
/**
 * Temporary stub for withdrawApplication to fix TS error
 * Replace this with your Redux action when ready
 */
const withdrawApplication = (jobId: string) => {
  console.log("Withdraw application for", jobId);
};
              ...
              <motion.div key={off.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
                <Card>
                  <CardContent className="p-4 md:p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge className={`px-3 py-1 text-xs ${off.status === "Received" ? "bg-blue-100 text-blue-800" : off.status === "Accepted" ? "bg-green-100 text-green-800" : off.status === "Declined" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}`}>
                        {off.status}
                      </Badge>
                      {off.status === "Received" && (
                        <>
                          <Button size="sm" onClick={() => handleAcceptOffer(off.id)}>Accept</Button>
                          <Button size="sm" variant="outline" onClick={() => handleDeclineOffer(off.id)}>Decline</Button>
                          <Button size="sm" variant="ghost" onClick={() => handleNegotiateOffer(off.id)}>Negotiate</Button>
                        </>
                      )}
                    </div>
                  </CardContent>
